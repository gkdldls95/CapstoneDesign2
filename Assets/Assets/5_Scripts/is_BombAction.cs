...
public class is_BombAction : MonoBehaviour
{
   public GameObject bombEffect;
    ...
    private int count =0; // 네트워크 지연시 효과 계속 생성 버그 방지
    void Update()
    {
        ...
        if (timer > 1.5 && count == 0)
        {
            ...
            //GameObject eff = PhotonNetwork.Instantiate("BombEffect", new Vector3(0, 0, 0), Quaternion.identity);
            eff.transform.position = transform.position;
            count++;


            Collider[] cols = Physics.OverlapSphere(transform.position, explosionRadius, 1 << 12);    // 12Layer
            ...
            Collider[] cols2 = Physics.OverlapSphere(transform.position, explosionRadius/2, 1 << 12);
